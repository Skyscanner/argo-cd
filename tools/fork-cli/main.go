...
func promoteFixCmd(args []string, runner CommandRunner) int {
	...
	releaseTag := parts[2]
	baseRelease := "skyscanner-internal/develop/" + releaseTag

	// 2) Fetch necessary refs
	runner.RunOrExit("git", "fetch", "origin", "skyscanner-contrib/master:skyscanner-contrib/master")
	runner.RunOrExit("git", "fetch", "origin", fmt.Sprintf("%s:%s", *fixBranch, *fixBranch))
	...
	if err != nil {
		if ciMode {
			printConflictMessage(os.Stderr, releaseTag, *fixBranch, *proposal)
			return 1
		}
