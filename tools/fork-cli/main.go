...
func (r *DefaultRunner) Run(cmdName string, args ...string) error {
	cmd := exec.Command(cmdName, args...) //nolint:noctx
	cmd.Stdout = r.stdout
	cmd.Stderr = r.stderr
	return cmd.Run()
}
...
func (r *DefaultRunner) BranchExists(ref string) bool {
	err := exec.Command("git", "rev-parse", "--verify", "--quiet", ref).Run() //nolint:noctx
	return err == nil
}

func (r *DefaultRunner) TagExists(tag string) bool {
	err := exec.Command("git", "rev-parse", "--verify", "--quiet", "tags/"+tag).Run() //nolint:noctx
	return err == nil
}
...
func syncForkCmd(args []string, runner CommandRunner) int {
	...
	runner.RunOrExit("git", "fetch", "origin", "--force", "--tags")
	runner.RunOrExit("git", "fetch", "upstream", "--force", "--tags")
	...
	runner.RunOrExit("git", "push", "--force", "--tags", "origin", contribBranch)
